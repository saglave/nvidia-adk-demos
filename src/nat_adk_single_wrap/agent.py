...
def update_session_state_callback(callback_context: CallbackContext) -> None:
    """Callback to update session state with current date and time before each tool call.
    Args:
        callback_context (CallbackContext): The callback context containing session state.
    Returns:
        None
    """
    ...
    callback_context.state['current_datetime'] = formatted_date
    logger.info(f"Updated session state with current date and time: {formatted_date}")

def read_session_state_callback(callback_context: CallbackContext) -> None:
    """Callback to read session state after each tool call.
    Args:
        callback_context (CallbackContext): The callback context containing session state.
    Returns:
        None
    """
    current_datetime = callback_context.state.get('current_datetime', 'Unknown')
    logger.info(f"Current date and time from session state: {current_datetime}")
...
async def adk_agent(config: ADKFunctionConfig, builder: Builder):
    ...
    tools = await builder.get_tools(sub_agent_config.tool_names, wrapper_type=LLMFrameworkEnum.ADK)
    sub_agent = Agent(
        name="adk_sub_agent",
        model = await builder.get_llm(
            sub_agent_config.llm, wrapper_type=LLMFrameworkEnum.ADK
        ),
